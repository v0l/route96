...
/// boolean → bool, integer → i64, float → f64, JSON array → sequence, otherwise string.
fn parse_value(raw: &str) -> Value {
    ...
    if let Ok(vals) = serde_json::from_str::<Vec<String>>(raw) {
        return Value::new(None, ValueKind::Sequence(vals.into_iter().map(|s| Value::new(None, ValueKind::String(s))).collect()));
    }
    // String fallback
