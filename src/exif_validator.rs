...
pub fn check_for_sensitive_exif(file_path: &Path) -> Result<()> {
    ...
    if has_device_info(&exifreader) {
        ...
        return Err(Error::msg(
            "Image contains device identification information",
        ));
    }
...
fn has_device_info(reader: &exif::Exif) -> bool {
    ...
    // Lens serial number
    if reader
        .get_field(exif::Tag::LensSerialNumber, exif::In::PRIMARY)
        .is_some()
    {
        return true;
    }
...
mod tests {
    ...
    fn test_jpeg_with_gps_rejected() {
        ...
        let jpeg_data = vec![
            ...
            // "Exif\0\0"
            0x45, 0x78, 0x69, 0x66, 0x00, 0x00, // TIFF header (little endian)
            ...
            // Next IFD
            0x00, 0x00, 0x00, 0x00, // Latitude values (dummy)
    ...
    fn test_jpeg_with_device_info_rejected() {
        ...
        let jpeg_data = vec![
            ...
            // Next IFD offset
            0x00, 0x00, 0x00, 0x00, // Make value: "Canon\0"
            0x43, 0x61, 0x6E, 0x6F, 0x6E, 0x00, 0xFF, 0xD9, // EOI
        ];
